...
Chickenwing uses `ffmpeg` for the clean parts of the job: audio extraction, remuxing, and post-processing.

What changed:

- Chickenwing now looks for `ffmpeg` in normal system `PATH`
- it also looks for the standard WinGet install location for `Gyan.FFmpeg`
- the next WinGet manifest generated by this repo declares `Gyan.FFmpeg` as a dependency

So the intended Windows install experience becomes:

```powershell
winget install Chyckenwing.Chickenwing
```

and WinGet should pull `ffmpeg` alongside Chickenwing.

If someone installs Chickenwing from GitHub with `pipx` instead of WinGet, they should still install `ffmpeg` separately.
